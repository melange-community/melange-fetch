...
module Headers = struct
  ...
  external delete : string -> t -> unit = "delete" [@@mel.send]
  (* entries *)
  (* very experimental *)

  external get : string -> t -> string option = "get"
  [@@mel.send] [@@mel.return { null_to_opt }]

  external has : string -> t -> bool = "has" [@@mel.send]
  (* keys *)
  (* very experimental *)

  external set : string -> string -> t -> unit = "set" [@@mel.send]
  (* values *)
  (* very experimental *)
end
...
module FormData = struct
  ...
  external append : string -> string -> t -> unit = "append" [@@mel.send]
  external delete : string -> t -> unit = "delete" [@@mel.send]
  external get : string -> t -> EntryValue.t option = "get" [@@mel.send]
  external getAll : string -> t -> EntryValue.t array = "getAll" [@@mel.send]
  external set : string -> string -> t -> unit = "set" [@@mel.send]
  external has : string -> t -> bool = "has" [@@mel.send]
  ...
  external setBlob : string -> blob -> ?filename:string -> t -> unit = "set"
  [@@mel.send]

  external setFile : string -> file -> ?filename:string -> t -> unit = "set"
  [@@mel.send]
